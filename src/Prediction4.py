...
def main():
    ...
    n_jobs = 10
    num_trees = 200
    estimator = RandomForestRegressor(n_estimators=num_trees, max_depth=None, verbose=1, n_jobs=2)
    min_features_to_select = 1007
    step = 50
    ...
    if need_fitting:
        rfecv = RFECV(estimator=estimator, step=step, cv=StratifiedKFold(5),
                      scoring='r2', verbose=2, n_jobs=n_jobs,
        ...
        save_object(rfecv, "../saved data/rfecv_"+str(n_training_samples))
        ...
        x_values = range(min_features_to_select, min_features_to_select + len(rfecv.grid_scores_) * step, step)
        y_values = rfecv.grid_scores_
        plt.plot(x_values, y_values)
        for a, b in zip(x_values, y_values):
            plt.text(a, b, format(b, '.5f'))
        plt.savefig("../saved data/cross_validation_rfecv_" + str(n_training_samples))
        draw_cross = time.perf_counter()
        print('finish drawing results of cross validation')
    else:
        rfecv = load_object("../saved data/rfecv_"+ str(n_training_samples))
        load_model_time = time.perf_counter()
        print('finish loading model, time: ' + str(load_model_time - sample_time))


    print('predicting ---------------')
    y_ = rfecv.predict(X_test)
    ...
    plt.scatter(x, y_)
    plt.savefig('../saved data/prediction_rfecv_'+ str(n_training_samples))
