...
def recursive_feature_elimination(test_size, label, df_dataset, estimator, rfe_dict,
                                  origin_n_features, target_n_features,
                                  save_rfe_dict=True,
                                  draw_prediction_pic=True, save_pic_fold='../saved data/rfe prediction/'):
    """
    ...
    support = rfe_dict[origin_n_features]['support']
    ranking = rfe_dict[origin_n_features]['ranking']
    ...
    if save_rfe_dict:
        save_object(rfe_dict, '../saved data/rfe_dict_'+str(n_training_samples))
        print('have saved rfe_dict_'+str(n_training_samples))

    save_pic_fold = save_pic_fold+'_'+str(n_training_samples)
    if draw_prediction_pic:
        if not os.path.exists(save_pic_fold):
            os.makedirs(save_pic_fold)
        plt.figure()
        ...
        plt.savefig(save_pic_fold + '/' + str(origin_n_features))
...
def main():
    ...
    # training
    data_portion = 0.8  # 340193 in total
    test_size = 0.25
    label = 'score'
    estimator = RandomForestRegressor(n_estimators=200, max_depth=None, verbose=1, n_jobs=8)
    save_rfe_dict = True
    draw_prediction_pic = True
    save_pic_fold = '../saved data/rfe_prediction'
    ...
    train, test = train_test_split(df_dataset, test_size=test_size)
    n_training_samples = train.shape[0]
    print()
    print('For further processing =================' )
    print('n_samples in total: ' + str(df_dataset.shape[0]))
    print('n_samples for training: ' + str(train.shape[0]))
    print('n_samples for testing : ' + str(test.shape[0]))
    ...
    n = [1907, 1000, 700, 500, 400, 300, 200, 100, 70, 60, 50, 40, 30, 20, 10, 5, 1]
    o_n = n[:-1]
    t_n = n[1:]
    for origin_n_features,target_n_features in zip(o_n, t_n):
        rfe_dict = recursive_feature_elimination(test_size=test_size, label=label, df_dataset=df_dataset,
                                                 estimator=estimator, rfe_dict=rfe_dict,
                                                 origin_n_features=origin_n_features, target_n_features=target_n_features,
                                                 save_rfe_dict=save_rfe_dict,
                                                 draw_prediction_pic=draw_prediction_pic, save_pic_fold=save_pic_fold)
