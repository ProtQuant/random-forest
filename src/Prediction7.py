...
def split_dateset(df_dataset, test_size=0.25, label='score'):
    ...
    test = copy.deepcopy(train).sort_values(by=[label])
    y_test = test[label]
...
def recursive_feature_elimination(X_train, y_train, X_test, y_test, estimator, rfe_dict,
    ...
    print(X_test.iloc[:, features].shape)
    print(X_test.iloc[:, features].columns.values.tolist())
    y_ = estimator.predict(X_test.iloc[:, features])
    score = r2_score(y_test, y_)
    rfe_dict[origin_n_features]['score'] = score
    print('r2_score on training set: ' + str(score))
    ...
    if save_rfe_dict:
        save_object(rfe_dict, save_data_folder + '/rfe3_dict_' + str(n_training_samples))
        print('have saved rfe3_dict_' + str(n_training_samples))
...
def main():
    ...
    # training
    data_portion = 0.5 # 340193 in total #0.3-76543 #0.5-127572 # 0.8-204116
    ...
    save_data_folder = save_data_folder + '/rfe3_' + str(n_training_samples)
    if not os.path.exists(save_data_folder):
        os.makedirs(save_data_folder)
    save_pic_folder = save_data_folder + '/rfe3_prediction_' + str(n_training_samples)
